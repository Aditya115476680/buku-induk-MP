...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h3 class="text-xl font-semibold text-gray-800 dark:text-white">Semester</h3>
            <p class="text-sm text-gray-500 dark:text-gray-400">Kelola data semester</p>
        </div>
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-gray-50 dark:bg-white/5 text-gray-600 dark:text-gray-200">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left font-semibold">Nama Semester</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/10">
                    @forelse($semesters as $s)
                        <tr class="text-gray-700 dark:text-gray-100">
                            <td class="px-4 py-3 whitespace-nowrap">{{ $s->academicYear->year ?? '-' }}</td>
                            <td class="px-4 py-3 font-medium">{{ $s->name }}</td>
                            <td class="px-4 py-3 text-right whitespace-nowrap space-x-2">
                                <a href="{{ route('semesters.edit', $s->id) }}"
                                   class="inline-flex items-center px-3 py-1.5 rounded-lg bg-gray-200 dark:bg-white/10 hover:opacity-90 transition">
                                    Edit
                                </a>

                                <form action="{{ route('semesters.destroy', $s->id) }}" method="POST" class="inline"
